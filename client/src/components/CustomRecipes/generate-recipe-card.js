      ...
      <div className={g.grnerate}>
        ...
        <form onSubmit={handleGenerate} className="space-y-4">
          ...
          <div className={g.dontknow}>
            ...
            <button type="submit" disabled={loading || !user} className={g.createbutton}>
              {loading ? (
                <>
          ...
          <div className="bg-white rounded-lg max-w-3xl w-full max-h-[90vh] overflow-y-auto">
            <div className="p-6">
              <div className="flex justify-between items-center mb-4">
                ...
                <button onClick={() => setOpen(false)} className={g.cross} aria-label="Close">
                  <FontAwesomeIcon icon={faXmark} />
                </button>
              </div>
              <div className={g.lordknows}>
                <MDEditor.Markdown source={recipeContent} className="bg-white p-4 rounded-md" />
              </div>
              <div className="flex justify-end gap-2 mt-4">
                <button onClick={() => setOpen(false)} className={`${g.closebox} ${g.recepiebutton}`}>
                  Close
                </button>
              </div>
            </div>
      ...
      <div className={g.saved}>
        <div className="mb-6">
          <h2 className="text-2xl font-bold text-gray-800 flex items-center">
            <FontAwesomeIcon icon={faBookmark} className="text-green-600 mr-2" />
            Your Saved Recipes
          </h2>
        </div>
        {!user ? (
          ...
          <div className="text-center py-8">
            ...
            <p className="text-sm text-gray-400 mt-2">Generate your first recipe above!</p>
          </div>
        ) : (
                ...
                <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4">
                  ...
                  <div className="flex gap-2">
                    <button
                      ...
                      className={`${g.savedbutton}`}
                    >
                      <FontAwesomeIcon icon={faEye} />
                      View
                    </button>
                    <button onClick={() => handleDelete(recipe._id)} className={`${g.delete} ${g.savedbutton}`}>
